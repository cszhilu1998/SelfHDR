...
if __name__ == '__main__':
	...
	for epoch in range(model.start_epoch + 1, opt.niter + opt.niter_decay + 1):
		...
		if opt.calc_metrics:
			...
			psnr_value = [0.0] * dataset_size_val
			time_val = 0
			for i, data in enumerate(tqdm_val):
				...
				psnr_value[i] = batch_psnr_mu(model.data_out, model.data_color_label, 1)
				# if opt.save_imgs:
				#     visualizer.display_current_results('val', res, epoch)
			visualizer.print_psnr(epoch, opt.niter + opt.niter_decay, time_val, np.mean(psnr_value))
